...
import AddClubForm from "./AddClubForm"; // Replace with the correct path to AddClubForm
...
const courses = [
  {
    ...
    action: "add-club", // Action identifier
  },
  {
    title: "Update Club",
    icon: <FaUserEdit className="w-10 h-10" />,
    description: "Update existing club details",
    navigateTo: "/update-club",
  },
  {
    title: "View Clubs",
    icon: <FaUser className="w-10 h-10" />,
    description: "View all club details",
    navigateTo: "/view-clubs",
  },
];

const Card = () => {
  const navigate = useNavigate();
  const [showAddClubPopup, setShowAddClubPopup] = useState(false);

  const handleCardClick = (actionOrRoute) => {
    if (actionOrRoute === "add-club") {
      setShowAddClubPopup(true);
    } else {
  ...
  return (
    <div>
      {/* Add Club Popup */}
      {showAddClubPopup && <AddClubForm onClose={() => setShowAddClubPopup(false)} />}
